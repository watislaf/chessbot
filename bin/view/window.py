...
class Window:
    ...
    def __drawBoard(self):
        for row in range(0, 8):
            for column in range(0, 8):
                ...
                self.__drawRect(((row + column) % 2 == 0), position_x, position_y)
        for row in range(0, 8):
            for column in range(0, 8):
                position_x = row * self.__cell_size
                position_y = column * self.__cell_size
                if self.__is_dragging and (column, row) == self.__position_dragging:
                    self.__drawPieces(self.__pieces.get(self.__fen[column][row], None),
                                      self.__mouse_x - self.__cell_size // 2, self.__mouse_y - self.__cell_size // 2)
                else:
                    self.__drawPieces(self.__pieces.get(self.__fen[column][row], None), position_x, position_y)

    def __drawRect(self, is_black: bool, position_x: int, position_y: int):
        if is_black:
    ...
    def __drawPieces(self, image: Surface, position_x: int, position_y: int):
        if image is None:
            return
