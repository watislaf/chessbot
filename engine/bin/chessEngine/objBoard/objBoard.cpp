...
void ObjBoard::setPiece(const Piece& piece_template_object) {
  ...
  if (board_[x][y] != NULL && board_[x][y]->getType() != PieceType::NONE) {
  ...
  if (piece_template_object.getType() == PieceType::NONE) {
    return;
  }
  if (board_[x][y]->getPieceColor() == ColorType::WHITE) {
  ...
  if (board_[x][y]->getType() == PieceType::KING) {
    if (board_[x][y]->getPieceColor() == ColorType::WHITE) {
...
void ObjBoard::apply(const Move& move) {
  ...
  if (move.isBrakeLeftCastle()) {
    setLCIsPossible(isWhiteTurn(), false);
  }
  if (move.isBrakeRightCastle()) {
    setRCIsPossible(isWhiteTurn(), false);
  }
  if (move.isCastle()) {
...
void ObjBoard::unApply(const Move& move) {
  ...
  if (move.isBrakeLeftCastle()) {
    setLCIsPossible(isWhiteTurn(), true);
  }
  if (move.isBrakeRightCastle()) {
    setRCIsPossible(isWhiteTurn(), true);
  }
  if (move.isCastle()) {
    if (move.getEnd()->getPosition().getX() < 3) {
      // LEFT RUCK PLACE
